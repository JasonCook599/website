...
async function updateTime() {
    ...
    //Add leading zeros.
    if (prefs.leadingzeros.hour) { var hour = hour.toString().padStart(2, 0) }
    if (prefs.leadingzeros.minute) { var minute = minute.toString().padStart(2, 0) }
    if (prefs.leadingzeros.second) { var second = second.toString().padStart(2, 0) }
/* 
    time=null
 ...
 */
    ...
    document.getElementById("second").textContent = second;
     document.querySelectorAll('.separator').forEach(function(element) {
        if (prefs.separator == null) { element.textContent = ":" } else { element.textContent = prefs.separator }
    }) 
